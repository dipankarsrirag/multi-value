...
def compute_predictions_logits(
    ...
    for example_index, example in enumerate(tqdm(all_examples, desc="Writing preditions")):
        ...
        score_yes, score_no, score_span, score_unk = (
            -float("INF"),
            -float("INF"),
            -float("INF"),
            float("INF"),
        ...
        (
            min_unk_feature_index,
            max_yes_feature_index,
            max_no_feature_index,
            max_span_feature_index,
        ) = (-1, -1, -1, -1)
        ...
        prelim_predictions.append(
            _PrelimPrediction(
                feature_index=min_unk_feature_index,
                start_index=0,
                end_index=0,
                score=score_unk,
                cls_idx=CLS_UNK,
        ...
        prelim_predictions.append(
            _PrelimPrediction(
                feature_index=max_yes_feature_index,
                start_index=0,
                end_index=0,
                score=score_yes,
                cls_idx=CLS_YES,
        ...
        prelim_predictions.append(
            _PrelimPrediction(
                feature_index=max_no_feature_index,
                start_index=0,
                end_index=0,
                score=score_no,
                cls_idx=CLS_NO,
...
def score(pred, truth, final_json):
    ...
    if no_total == 0:
        ...
        no_f1 = 100.0 * no_f1 / no_total
    if yes_total == 0:
        yes_f1 = 0
    else:
        yes_f1 = 100.0 * yes_f1 / yes_total
    if no_ans_total == 0:
        no_ans_f1 = 0.0
    else:
        no_ans_f1 = 100.0 * no_ans_f1 / no_ans_total
    normal_f1 = 100.0 * normal_f1 / normal_total
    result = {
        "total": total,
        "f1": f1,
        "no_total": no_total,
...
def score_each_instance(pred, truth):
    ...
    f1_scores = [100.0 * x / total for x in f1_scores]
    return f1_scores
