...
def coqa_convert_example_to_features(example, tokenizer, max_seq_length, doc_stride, max_query_length):
    ...
    for doc_span_index, doc_span in enumerate(doc_spans):
        ...
        out_of_span = False
        if example["rationale_start_position"] == -1 or not (
            tok_r_start_position >= doc_start and tok_r_end_position <= doc_end
        ):
            out_of_span = True
        if out_of_span:
            rationale_start_position = 0
            rationale_end_position = 0
        else:
            doc_offset = len(query_tokens) + 2
            rationale_start_position = tok_r_start_position - doc_start + doc_offset
            rationale_end_position = tok_r_end_position - doc_start + doc_offset
        # rationale_part_end
...
def coqa_convert_examples_to_features(
    examples,
    tokenizer,
    max_seq_length,
    doc_stride,
    ...
    with Pool(
        threads,
        initializer=coqa_convert_example_to_features_init,
        initargs=(tokenizer,),
    ) as p:
        annotate_ = partial(
...
class CoqaProcessor(DataProcessor):
    ...
    def get_examples(self, data_dir, history_len, filename=None, threads=1):
        ...
        examples = [item.__dict__ for sublist in examples for item in sublist]
        examples = datasets.Dataset.from_pandas(pd.DataFrame(data=examples))
        return examples
        ...
        for i in range(len(datum["questions"])):
            ...
            if input_text in chosen_text:
                ...
                _qas["answer_span"] = self.find_span(
                    _datum["raw_context_offsets"],
                    start + p,
                    start + p + len(input_text),
                )
            else:
                _qas["answer_span"] = self.find_span_with_gt(
                    _datum["context"], _datum["raw_context_offsets"], input_text
                )
